...
public:
    ...
    __aicore__ inline void Init(GM_ADDR expandX, GM_ADDR expertIds, GM_ADDR expandIdx, GM_ADDR epSendCount,
                                GM_ADDR tpSendCount, GM_ADDR scales, GM_ADDR xActiveMask, GM_ADDR commArgs, GM_ADDR XOut,
...
private:
    ...
    {
#if ENABLE_CAM_COMM
        return (GM_ADDR)epWinContext_.GetValue(rankId) + Moe::IPC_DATA_OFFSET + winDataSizeOffset_ +
    ...
    {
#if ENABLE_CAM_COMM
        return (GM_ADDR)epWinContext_.GetValue(rankId) + dataState_ * WIN_STATE_OFFSET;
#else
        if (domain == EP_DOMAIN) {
            return (GM_ADDR)((epRankId_ == rankId)
...
#if ENABLE_CAM_COMM
    AscendC::GlobalTensor<GM_ADDR> epWinContext_;
    AscendC::GlobalTensor<GM_ADDR> tpWinContext_;
#else
...
__aicore__ inline void CamMoeDistributeCombine<TemplateMC2TypeFunc>::Init(
    ...
    GM_ADDR xActiveMask, GM_ADDR commArgs, GM_ADDR XOut, GM_ADDR workspaceGM, TPipe *pipe, const FusedDeepMoeTilingData *tilingData)
{
    tpipe_ = pipe;
    ...
    epWinContext_.SetGlobalBuffer(
            &(reinterpret_cast<__gm__ Moe::CommArgs *>(commArgs))->peerMems[0], Moe::CAM_MAX_RANK_SIZE);
    GM_ADDR statusDataSpaceGm = (GM_ADDR)epWinContext_.GetValue(epRankId_);
#else
...
{
    MoeTracing(TRACE_POINT("gmm2-combine combine-barrier-all", "B"));
    MoeTracing<true>(TRACE_POINT("gmm2-combine combine-barrier-all", "E"));
    MoeTracing(TRACE_POINT("gmm2-combine combine-wait-send", "B"));
    AscendC::CrossCoreSetFlag<0x0, PIPE_MTE3>(SEND_SYNC_EVENT_ID);
    AscendC::CrossCoreWaitFlag(SEND_SYNC_EVENT_ID);
    MoeTracing(TRACE_POINT("gmm2-combine combine-wait-send", "E"));
    MoeTracing(TRACE_POINT("gmm2-combine combine-set-status", "B"));
    if constexpr ((EXEC_FLAG & EXEC_FLAG_DEEP_FUSE) == 0) {
        BuffInit();
        SetWaitTpStatusAndDisPatch();
    }
    AlltoAllBuffInit();
    SetStatus();
    MoeTracing(TRACE_POINT("gmm2-combine combine-set-status", "E"));
    MoeTracing(TRACE_POINT("gmm2-combine combine-wait-status", "B"));
    WaitDispatch();
    MoeTracing(TRACE_POINT("gmm2-combine combine-wait-status", "E"));
    MoeTracing(TRACE_POINT("gmm2-combine combine-local-copy", "B"));
    LocalWindowCopy();
    MoeTracing(TRACE_POINT("gmm2-combine combine-local-copy", "E"));
}
}  // namespace MoeDistributeCombineImpl
