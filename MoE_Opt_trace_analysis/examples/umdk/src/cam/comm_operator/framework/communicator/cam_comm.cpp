...
int CamComm::Init()
{
    ...
    if (SyncCommArgs() != CAM_SUCCESS) {CAM_LOG(ERROR) << "SyncCommArgs failed!"; return CAM_ERROR_INTERNAL;}
...
int CamComm::EnablePeerAccess()
{
    ...
    for (int dev = 0; dev < localRankSize_; ++dev) {
        if (localRank_ == dev) {
            continue;
        }

        aclError ret = rtEnableP2P(devId_, dev, 0);
        if (ret != ACL_SUCCESS) {
            CAM_LOG(ERROR) << "err rtEnableP2P failed peerDeviceId = " << dev << ",rank= " << rank_
                           << ", flags = " << 0 << "," << __LINE__ << ":" << ret;
            return CAM_ERROR_INTERNAL;
        }
    }
...
int CamComm::InitMem()
{
    ...
    aclError memInfoRes = aclrtGetMemInfo(ACL_DDR_MEM, &freeSize, &totalSize);
    CAM_LOG(INFO) << "check mem info " << __FILE__ << ":" <<__LINE__ << ": " << "memInfoRes= " << memInfoRes << ", totalSize= " << totalSize << ", freeSize= " << freeSize;
