...
def _phase_category(phase: str, name: str, mapper: Any = None) -> str:
    ...
    if "block-epilogue" in text:
        return "epilogue"
    if "send" in text or "recv" in text or "combine" in text or "dispatch" in text or "local-copy" in text:
        return "communication"
    if "aic" in text or "aiv" in text or "compcorefunc" in text or "moe-process" in text:
        return "compute"
...
def build_phase_summary(instances: Table) -> Table:
    ...
    rows = [_summarize_rows("phase", phase, group, total_wall) for phase, group in _group_by(instances, "phase").items()]
...
def build_name_summary(instances: Table) -> Table:
    ...
    rows.sort(key=lambda r: (_float(r.get("total_us")), _float(r.get("union_us")), int(r.get("count", 0))), reverse=True)
    return rows
