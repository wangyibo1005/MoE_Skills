...
def build_statistical_summary(
    ...
    if category_core_group_summary:
        ...
        for core_group in core_groups:
            group_rows = [
                row for row in category_core_group_summary
                if row.get("core_group") == core_group
                and row.get("category") not in {None, "", "container"}
            ]
            if not group_rows:
                continue
...
def build_markdown_report(
    ...
    low_overlap = sorted(
        [
            row for row in overlap_summary
            if row.get("overlap_ratio_min", 0.0) <= 0.2
            and min(row.get("union_us_a", 0.0), row.get("union_us_b", 0.0)) > 0
        ],
        key=lambda r: (min(r.get("union_us_a", 0.0), r.get("union_us_b", 0.0)), -r.get("overlap_us", 0.0)),
        reverse=True,
    )
