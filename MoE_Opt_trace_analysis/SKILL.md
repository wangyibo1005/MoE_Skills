---
name: MoE_Opt-trace-analysis
description: >-
  MoE_Opt_trace_analysis（Skill 3，编排 Step 4）：解析 Chrome/Perfetto trace.json，按 phase、category、core group、tid 统计耗时、overlap、bubble；
  输出确定性诊断报告，可选外部 LLM 扩写。
---

# MoE_Opt_trace_analysis · Trace 解析与归因

## 与工作流衔接

本目录对应 **`moe_closed_loop_workflow.md`** 中 **Step 4**：输入一般为 **`candidate_trace`** 工况下得到的 **`trace.json`**（Chrome/Perfetto 格式）。

## 作用
分析 Chrome/Perfetto 风格的 `trace.json`，把原始 trace event 转换为结构化统计表、图表和 Markdown 报告，用于替代人工在 Perfetto 中做第一轮耗时分布和瓶颈定位。

这个 Skill 面向的核心对象是 `trace.json`，不是某一个固定算子。它本身负责：
- 解析 trace 中的完整区间事件。
...
## Agent 执行原则
执行这个 Skill 时，agent 不应把文档中的示例路径当成固定输入。应先从用户请求或当前工作区中确认以下上下文，并把它们替换到命令中：
...
## Phase 和 Category
Skill 通过 `--phase-map` 指定的 YAML 配置把原始 trace name 映射到稳定 phase。配置包含两类信息：
...
如果 trace event args 中没有 `core_type/core_id`，Skill 会尝试从 `tid` 推断，例如 `type1_core003 -> vector_recv/core_id=3`。
